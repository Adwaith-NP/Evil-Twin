...
def displayWifi(interface):
    ...
    def get_wifi_list():
        try:
            command = "nmcli -f SSID,BSSID,CHAN,SIGNAL,SECURITY device wifi list ifname {}".format(interface)
            res = subprocess.check_output(command, encoding="utf-8",shell=True)
            return res
    ...
    def printAndSelect(stringList):
        ...
        return stringList[selection-1][1],stringList[selection-1][2],stringList[selection-1][0]


        

    listAsPerLastOut = splitAndStore(lastOutput)
    Bssid,channel,name = printAndSelect(listAsPerLastOut)
